...
import { SurahIntroModal } from '../components/SurahIntroModal';
...
export const SurahScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  // Bookmark drawer states
  const [bookmarkDrawerVisible, setBookmarkDrawerVisible] = useState(false);
  const [bookmarkTargetAyah, setBookmarkTargetAyah] = useState<{ surah: number; ayah: number } | null>(null);
  ...
  }, [navigation, colors, isSurahMode, surahId]);
  ...
  useEffect(() => {
    if (surahId) {
      ...
      // Start downloading Tafseer paused for now
      // tafseerService.startSurahDownload(surahId);
    } else if (juzNumber) {
      ...
      if (juzAyahs.length > 0) {
        ...
        // Start downloading Tafseer paused for now
        // tafseerService.startSurahDownload(juzAyahs[0].surah);
      }
    }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // Clear the parameter so it doesn't trigger scroll again on tab switches
      navigation.setParams({ initialAyah: undefined } as any);
    }, 280);
    return () => clearTimeout(timer);
  }, [initialAyah, listData, jumpToIndex, navigation]);

  const refreshBookmarksState = useCallback(() => {
    const set = new Set<string>();
    for (const b of bookmarkService.getBookmarks()) {
      set.add(`${b.surah}:${b.ayah}`);
    }
  ...
  const handleToggleBookmark = useCallback(
    async (surah: number, ayah: number) => {
      const isBookmarked = bookmarkService.isBookmarked(surah, ayah);
      if (isBookmarked) {
        await bookmarkService.removeBookmark(surah, ayah);
        refreshBookmarksState();
        Alert.alert('Bookmark Removed', 'The bookmark has been removed.');
      } else {
        setBookmarkTargetAyah({ surah, ayah });
        setBookmarkDrawerVisible(true);
      }
    },
    [refreshBookmarksState]
  );
    ...
    ({ item }: { item: ListItem }) => {
      ...
      return (
        <AyahWidget
          ...
          onOpenTafseer={() => {
            setSelectedAyahForTafseer(item);
          ...
          colors={colors}
          enableHifzBlur={preferences.enableHifzBlur}
        />
      );
    },
    [bookmarkedKeys, handleToggleBookmark, preferences.translationLanguage, preferences.arabicFont, preferences.enableHifzBlur, colors]
  );
  ...
  return (
    <Animated.View style={[styles.container, { backgroundColor: colors.background, opacity: fadeAnim }]}>
      ...
      {/* Tafseer Exegesis Bottom Drawer */}
      <TafseerDrawer
        visible={tafseerVisible}
        onClose={() => setTafseerVisible(false)}
        ayah={selectedAyahForTafseer}
        colors={colors}
      />

      {/* Bookmark Drawer (Bottom Sheet) */}
      <BookmarkDrawer
        visible={bookmarkDrawerVisible}
        onClose={() => setBookmarkDrawerVisible(false)}
        surahId={bookmarkTargetAyah?.surah ?? 1}
        ayahNumber={bookmarkTargetAyah?.ayah ?? 1}
        juzNumber={juzNumber}
        colors={colors}
        onBookmarkSaved={(savedType, folderName) => {
          refreshBookmarksState();
          if (savedType === 'folder') {
            Alert.alert('Bookmark Saved', `Added to folder "${folderName}".`);
          } else if (savedType === 'replaced') {
            Alert.alert('Bookmark Replaced', 'Replaced the last active bookmark.');
          } else {
            Alert.alert('Bookmark Saved', 'Saved to general bookmarks.');
          }
        }}
      />
      ...
      {isSurahMode && surahId && (
        <SurahIntroModal
          ...
          surahId={surahId}
          surahNameTranslit={surahMeta?.name_translit ?? ''}
          colors={colors}
        />
      )}
    </Animated.View>
